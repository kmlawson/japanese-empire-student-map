...
(function () {
  ...
  window.JMAP_ANNOTATE = function (host) {
    ...
    var ANN_MAX_BYTES = 24 * 1024 * 1024;   // a bigger file is refused
    var ANN_MAX_VERTS = 240000;             // and so is one with more points
    ...
    function changed(quiet) {
      ...
      // the packed link is worked out off the critical path, so that pressing
      // Copy link is a synchronous copy inside the reader's own click — see
      // copyLink
      window.setTimeout(prepLink, 0);
    }

    function store() {
      try {
        if (!feats.length) window.localStorage.removeItem(ANN_STORE);
        else window.localStorage.setItem(ANN_STORE,
    ...
    function props(kind, st) {
      ...
      if (kind === 'point' || kind === 'event') {
        ...
        p['marker-symbol'] = kind === 'event' ? 'diamond' : st.symbol;
      }
      if (kind === 'polygon') {
        p.fill = st.colour;
    ...
    function kindOf(f) {
      ...
      if (t === 'Point' || t === 'MultiPoint') {
        return ((f.properties || {})['marker-symbol']) === 'diamond' ? 'event' : 'point';
      }
    ...
    function markerShape(symbol, r, colour) {
      if (symbol === 'diamond') {
        return host.svgEl('path', { d: 'M0 ' + (-r) + 'L' + r + ' 0L0 ' + r + 'L' + (-r) + ' 0Z',
          fill: colour, stroke: '#fffdf8', 'stroke-width': 1.2 });
      }
      if (symbol === 'square') {
        return host.svgEl('rect', { x: -r * 0.85, y: -r * 0.85, width: r * 1.7, height: r * 1.7,
          fill: colour, stroke: '#fffdf8', 'stroke-width': 1.2 });
      }
      if (symbol === 'triangle') {
        return host.svgEl('path', { d: 'M0 ' + (-r * 1.1) + 'L' + (r) + ' ' + (r * 0.8)
          + 'L' + (-r) + ' ' + (r * 0.8) + 'Z',
          fill: colour, stroke: '#fffdf8', 'stroke-width': 1.2 });
      }
      return host.svgEl('circle', { r: r, fill: colour, stroke: '#fffdf8', 'stroke-width': 1.2 });
    }

    function addMarker(symbol, lon, lat, colour, size, cls, meta) {
    ...
    function grab(target, cx, cy) {
      if (!on || tool) return false;
      var el = target && target.closest ? target.closest('[data-ann]') : null;
      if (!el || !el.classList.contains('ann-mark')) return false;
      ...
      if (!feats[i] || isNaN(r) || isNaN(v)) return false;
      snapshot();
      dragging = { i: i, r: r, v: v, moved: false };
      sel = i;
      syncFields();
      drawList();
      redraw();
    ...
    function syncFields() {
      ...
      if (f) {
        ...
        var col = $('#ann-colour'), sz = $('#ann-size'),
            fill = $('#ann-fill'), dash = $('#ann-dash'), sym = $('#ann-symbol');
        ...
        if (fill && p['fill-opacity'] !== undefined) fill.checked = p['fill-opacity'] > 0;
    ...
    function styleChanged() {
      ...
      if (kind === 'point' || kind === 'event') {
        p['marker-color'] = st.colour;
        p['marker-size'] = st.size <= 2 ? 'small' : (st.size >= 5 ? 'large' : 'medium');
        // an event stays a diamond; a point takes whichever symbol is chosen
        if (kind === 'point') p['marker-symbol'] = st.symbol;
      }
      if (kind === 'polygon') {
        p.fill = st.colour;
        p['fill-opacity'] = st.fill ? 0.28 : 0;
    ...
    function loadText(text, name, merge) {
      ...
      if (verts > ANN_MAX_VERTS) {
        say('That would come to ' + verts.toLocaleString() + ' points, past the '
    ...
    function loadFile(file, merge) {
      ...
      if (file.size > ANN_MAX_BYTES) {
        say('That file is ' + Math.round(file.size / 1048576) + ' MB, past the '
    ...
    /* THE LINK IS PACKED BEFORE IT IS ASKED FOR, and this is the whole reason
       ...
       everywhere. */
    function prepLink() {
      if (!linkDirty || !feats.length) return;
      var mine = feats;
      pack(collection()).then(function (code) {
        ...
        if (b) {
          b.disabled = false;
          b.title = code.length > ANN_URL_MAX
            ? 'Too much to put in a link — save the file instead'
    ...
    var PANEL = '' +
      ...
      '<div class="ann-body">' +
        '<div class="ann-tools" role="group" aria-label="Drawing tools">' +
          ...
          '<button type="button" class="ann-tool" data-tool="event" aria-pressed="false">Event</button>' +
        ...
        '<div class="ann-style">' +
          ...
          '<label>Shape <select id="ann-symbol">' +
            '<option value="circle">Dot</option><option value="square">Square</option>' +
            '<option value="triangle">Triangle</option><option value="diamond">Diamond</option>' +
          '</select></label>' +
          '<label class="ann-check"><input type="checkbox" id="ann-fill" checked> Fill areas</label>' +
          '<label class="ann-check"><input type="checkbox" id="ann-dash"> Dashed lines</label>' +
    ...
    function build() {
      ...
      ['ann-colour', 'ann-size', 'ann-fill', 'ann-dash', 'ann-symbol'].forEach(function (id) {
        var el = $('#' + id, panel);
    ...
    function fromUrl(code) {
      open(true);                 // a reader who followed a link came to look
      unpack(code).then(function (text) {
        loadText(text, 'shared');
        // it was opened folded on purpose: the marks are the point, the tools
        // are there if wanted. The message says where they are.
        say('These annotations came with the link. Open the panel above to add '
          + 'to them or save them.');
      }, function (err) {
        fold(false);
