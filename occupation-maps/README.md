# Reference maps of the Japanese occupation

Downloaded to work out the shape of the occupied zone in China. They are
reference material, not part of the map that is served: nothing here is
redistributed by the page itself, and each file keeps the licence it came
with. The traced result lives in `OCCUPIED_ZONE` in `tools/build_map.py`.

The main source is the 1940 sheet from the *China 1900–1949* series, which is
in the public domain and shows the occupied area as it really was — the
plains, the railways and the cities, not whole provinces. It was adjusted to
December 1942 against the module's Sino-Japanese War timeline and against the
campaign histories. Changsha held out until 1944. Foochow was held for a
fortnight in 1941. Chengchow was taken in October 1941 but given up again at
the end of that month and not held for good until April 1944, so it is outside
the line, and so is Loyang, which held until May 1944. The Chekiang gains of
mid-1942 were largely evacuated that autumn. Kwangchowwan was not taken until
February 1943.

The timeline card that says "Zhengzhou falls, 1941 Oct" is what first put
Chengchow inside the line; it records the capture but not the withdrawal.

## Files

* `German_and_Japanese_spheres_of_influence_at_greatest_extent_World_War_II_1942.jpg`
  * Wikimedia Commons: [File:German and Japanese spheres of influence at greatest extent World War II 1942.jpg](https://commons.wikimedia.org/wiki/File:German_and_Japanese_spheres_of_influence_at_greatest_extent_World_War_II_1942.jpg)
  * Licence: CC BY-SA 3.0
  * Author: Ww2_allied_axis.gif: User:Hohum derivative work: 36ophiuchi (talk)

* `Japanese_Occupation_1940_updated.png`
  * Wikimedia Commons: [File:Japanese Occupation 1940 updated.png](https://commons.wikimedia.org/wiki/File:Japanese_Occupation_1940_updated.png)
  * Licence: Public domain
  * Author: U.S. Army

* `Japanese_Occupation_of_China_1940-fa.png`
  * Wikimedia Commons: [File:Japanese Occupation of China 1940-fa.png](https://commons.wikimedia.org/wiki/File:Japanese_Occupation_of_China_1940-fa.png)
  * Licence: CC BY-SA 4.0
  * Author: Roozitaa

* `Japanese_Occupation_of_China_1940_(Chongqing).png`
  * Wikimedia Commons: [File:Japanese Occupation of China 1940 (Chongqing).png](https://commons.wikimedia.org/wiki/File:Japanese_Occupation_of_China_1940_(Chongqing).png)
  * Licence: CC BY-SA 3.0
  * Author: Odie5533

* `Second_Sino-Japanese_War_WW2.png`
  * Wikimedia Commons: [File:Second Sino-Japanese War WW2.png](https://commons.wikimedia.org/wiki/File:Second_Sino-Japanese_War_WW2.png)
  * Licence: Public domain
  * Author: U.S. Army

## Chahar and Suiyuan

A second pass over the north-west corner of the zone, written up in
`report.md` with its references in `sources.md`. `range-report.md` breaks the
same material down year by year — which large settlements were held, and
whether that is confirmed by a dated capture, likely from county and league
membership, or merely claimed on paper. Both are plotted in
`suiyuan-history.html`, a standalone Leaflet page: events filterable by year and
steppable one at a time, with the control zones for the year in view drawn
underneath them. The finding that matters for
the line: Japanese control in Suiyuan ended at Baotou from 17 October 1937 to
August 1945, apart from six weeks in February and March 1940, and the Ordos
south of the Yellow River was never occupied at all. The maps below were
downloaded for that work.

* `Mengjiang_saishin_zenzu_1938_front.jpg`, `Mengjiang_saishin_zenzu_1938_back.jpg`
  * `蒙疆最新全圖`, 1:2,000,000, 54×79 cm, supplement to the *Mengjiang
    Shimbun* of 22 November 1938. The Mengjiang regime's own general map; the
    reverse carries the paper's first-anniversary pages and the government
    organisation chart.
  * Source: [Nichibunken map database, id 004808051](https://lapis.nichibun.ac.jp/chizu/map_detail.php?id=004808051)
  * Licence: public domain in Japan (published 1938)
  * Author: 蒙疆新聞社ヵ

* `Mokyo_Moko_rengo_map_1938_Yomiuri.jpg`, `Hokushi_map_1938_Yomiuri.jpg`
  * `蒙疆：蒙古聯盟` and `北支那：中華民國臨時政府ノ部`, the two sides of one
    sheet, 1:2,400,000, 40×55 cm, November 1938.
  * Source: [Nichibunken map database, id 003509502-0003](https://lapis.nichibun.ac.jp/chizu/map_detail.php?id=003509502-0003)
  * Licence: public domain in Japan (published 1938)
  * Author: 讀賣新聞社編輯局 / アトラス社

* `Zhangjiakou_city_plan_Mengjiang.jpg`
  * `張家口市街圖`, 1938, passed by the Hasunuma force censor. City plan of
    the Mengjiang capital.
  * Source: [Nichibunken map database, id 004975512](https://lapis.nichibun.ac.jp/chizu/map_detail.php?id=004975512)
  * Licence: public domain in Japan (published 1938)
  * Author: 蒙疆新聞社

* `Hokushi_Mokyo_railway_map_1939_Kahoku_Kotsu.jpg`
  * `北支蒙疆 鉄道略図`, July 1939. The railway network the occupied zone was
    drawn around.
  * Wikimedia Commons: [File:北支蒙疆 铁道略图-华北交通株式会社.jpg](https://commons.wikimedia.org/wiki/File:%E5%8C%97%E6%94%AF%E8%92%99%E7%96%86_%E9%93%81%E9%81%93%E7%95%A5%E5%9B%BE-%E5%8D%8E%E5%8C%97%E4%BA%A4%E9%80%9A%E6%A0%AA%E5%BC%8F%E4%BC%9A%E7%A4%BE.jpg)
  * Licence: Public domain
  * Author: 華北交通株式会社

* `AMS_L500_NK49-7_Wu-yuan.jpg`, `AMS_L500_NK49-11_Kuei-sui.jpg`
  * AMS Series L500, China 1:250,000, sheets NK 49-7 Wu-yuan and NK 49-11
    Kuei-sui. Terrain of the Hetao front and of the Daqingshan.
  * Source: [Perry-Castañeda Library, University of Texas at Austin](https://maps.lib.utexas.edu/maps/ams/china/)
  * Licence: Public domain
  * Author: U.S. Army Map Service

* `Mengjiang_North_Shanxi_South_Chahar_Map_c1937-1939.png`
  * Wikimedia Commons: [File:Mengjiang, North Shanxi, South Chahar Map - Circa 1937-1939.png](https://commons.wikimedia.org/wiki/File:Mengjiang,_North_Shanxi,_South_Chahar_Map_-_Circa_1937-1939.png)
  * Licence: Public domain
  * Author: Unknown

* `Administrative_map_of_the_United_Mongol_Autonomous_Government.png`
  * Wikimedia Commons: [File:Administrative map of the United Mongol Autonomous Government.png](https://commons.wikimedia.org/wiki/File:Administrative_map_of_the_United_Mongol_Autonomous_Government.png)
  * Licence: CC BY-SA 4.0
  * Author: Khu'hamgaba Kitap

* `South_Chahar_Autonomous_Government_Map_c._1937.png`
  * Wikimedia Commons: [File:South Chahar Autonomous Government Map c. 1937.png](https://commons.wikimedia.org/wiki/File:South_Chahar_Autonomous_Government_Map_c._1937.png)
  * Licence: CC BY-SA 4.0
  * Author: Khu'hamgaba Kitap

* `North_Shanxi_Autonomous_Government_Map_1937.png`
  * Wikimedia Commons: [File:North Shanxi Autonomous Government Map, 1937.png](https://commons.wikimedia.org/wiki/File:North_Shanxi_Autonomous_Government_Map,_1937.png)
  * Licence: CC BY-SA 4.0
  * Author: Khu'hamgaba Kitap

* `Japanese_zones_of_control.png`
  * From *Why We Fight VI: The Battle of China*, 1944 — an American wartime
    reading of actual as against claimed control.
  * Wikimedia Commons: [File:Japanese zones of control.png](https://commons.wikimedia.org/wiki/File:Japanese_zones_of_control.png)
  * Licence: Public domain
  * Author: Walt Disney Studios with the U.S. Office of War Information

## Timeline used alongside them

The Sino-Japanese War card set from the module's own timeline review tool,
which gives the dates the cities fell: Peking and Tientsin July 1937;
Shanghai November 1937; Taiyuan November 1937; Nanking December 1937;
Hsuchow May 1938; Amoy May 1938; Wuhan and Canton October 1938; Swatow June
1939; Hainan February 1939; Nanchang March 1939; Ichang June 1940;
Chengchow October 1941, given up again that month; Hong Kong December 1941.
Changsha was attacked repeatedly and did not fall until June 1944, and Loyang
not until May 1944.

## The caveat that belongs on the map

The shading marks where Japanese authority reached, not where it went
unchallenged. Communist base areas — Jin-Cha-Ji, Ji-Lu-Yu — operated inside
the line throughout, which is why the Japanese army ran repeated "mopping up"
campaigns against them in 1943, and Nationalist guerrillas held ground behind
it as well. The map says so in the note on the occupied zone.
