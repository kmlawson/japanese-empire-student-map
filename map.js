...
(function () {
  ...
  /* **Any kana at all disqualifies a field, not merely kana throughout.**
   *
   * The first rule here refused a reading only when it was kana from end to
   * end, which let a great deal of katakana through on the back of a single
   * character: オホーツク海, ベンガル湾, タクラマカン砂漠, ハイラル区. The switch
   * is called Kanji/Hanzi/Hanja and katakana is none of the three -- and in
   * every one of those cases `zh` was sitting behind it holding the actual
   * characters, 鄂霍次克海 and 孟加拉灣 and 塔克拉瑪干沙漠 and 海拉爾. Sixty-six
   * fields across the tables were like that, and the switch was printing kana
   * for all of them.
   *
   * `ヶ` is the one exception and it is a real one: it is written in kana but
   * it belongs to the name, and 青ヶ島 and 霞ヶ浦 have no other spelling. It and
   * the interpunct and the long vowel mark are set aside before the test
  ...
  function hanOf(rec) {
    ...
    var r = shown(rec);
    /* **The pre-war form where the file has one.** `ja` is modern shinjitai —
       金沢, 豊橋 — and the switch was asked for 旧字 on Japan and Karafuto.
       `ja_kyu` carries that, on the eleven names where it differs and the
       one-to-one table can be trusted; `build_texts.py` refuses to build
       unless each one converts back to its own `ja`. A name without it keeps
       the modern form, which is the "when available" contract again, and is
       why nothing outside Japan is touched by this line. */
  ...
  var TRAIN_SYS = {
    tw: {
      ...
      /* The book itself, named as it is named, with the scan behind it. A
         sentence about the railway was a sentence the reader has to take on
         trust; the title and the link are the thing they can check. */
      src: '\u81fa\u7063\u9435\u9053\u6642\u523b\u8868 (1936)',
  ...
  function gazEnrich(c) {
    ...
    if (b) {
      // `ja_kyu` rides with the rest: the dot's label is drawn from this
      // merged record, so a field left out here is a field the map never sees
  ...
  function buildSiteLabels() {
    ...
    buildStations = function (sys) {
      ...
      /* The table is fetched the first time its layer is switched on. The
         three station files were loaded with the page — 300 KB, 59 gzipped —
         for layers that start off and that most readers never open. A
         `?layers=` code with the stations on lands here from `applyState`
         before the table is in, asks for it, and comes back through this
         function when it arrives; nothing else has to know. */
      if (!JMAP[cfg.data]) {
  ...
  function handleTap(target, cx, cy, sticky) {
    ...
    if (tHit && tHit.kind === 'line' && !(hit && hit.rec.kind === 'station')) {
      ...
      if (lCard) {
        /* **Pressing the track picks the line, the way its name in the strip
           does.** The two are the same act — "this one, not the other forty" —
           and it would be odd for the chip to light the map and the line
           itself not to. */
    ...
    /* And a press anywhere that is not a line lets it go again. A picked line
       dims everything else on the map, so leaving it lit after the reader has
       moved on is the map holding a selection they have stopped making. */
  ...
  JMAP.__airShortNames = airShortNames;
    ...
    JMAP.AIR.forEach(function (r) {
      ...
      /* **A line may carry its own colour.** The Japanese network is one ink
         ...
         so a reader can see at a glance that these are two systems and not one
         with a gap in it. Set as a custom property on the group so the line,
         its hover state and the aeroplanes that fly it all take it from the
         same place. */
  ...
  function railFileName(f) {
    ...
    /* The English name, not the displayed one: with the characters switch on
       `line` is 京釜線, and `slug` would take every character out and leave the
       file called after nothing at all. */
  ...
  function applyState() {
    ...
    if (trainApi && trainApi.mounted()) {
      trainApi.recolour();
      // and the names, which follow the Japanese-names switch and the
      // characters one
      trainApi.renamed();
      if (selected && byId[selected]) fillTrainCard(byId[selected]);
    }
  ...
  function fillPopCard(key, name) {
    ...
    if (sets.length && !(sets[0].rows || {})[k]) {
      ...
      /* **The other way a place can carry somebody else's figures**, and the
         one this was missing. Above is the island with no row at all, which
         reads its province's. Here the source *does* print a row — Cheju is in
         the table — and the row says `same_as`, meaning the census counted its
         people inside another unit and the build copied that unit's numbers
         in. The figures are identical either way and the heading was not: a
         missing row was headed with the province and a present one with
         itself, so Cheju stood over Chŏllanam-do's 2,332,256 as though a
         hundred-kilometre island held eleven per cent of Korea.

         Reported: the card is right about Cheju and the table over it has to
         say whose count that is. The row is left as the one that is read —
         it already holds the copied figures — and only the name over it
         changes, so nothing about which numbers are shown is touched. */
    ...
    sets.forEach(function (d) {
      ...
      /* Said once. The `same_as` rows carry a note of their own out of the
         source table — "The 1930 census counts Cheju inside Chŏllanam-do and
         does not break the island out" — which is this sentence in the
         dataset's own more exact words, and it is rendered below. Adding this
         one as well printed the same caveat twice under the same heading. The
         island with no row at all has no such note, and still needs one. */
      if (borrowed && !r.note) {
  ...
  function wireControls() {
    ...
    document.addEventListener('keydown', function (e) {
      ...
      /* And the air network. Unlike the railway button this one is offered at
         every zoom and on both dates — the five services cross the whole map,
