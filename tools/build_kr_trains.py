...
LINE_EN = {
    '京釜本線': 'Kyŏngbu Line',
    '京義本線': 'Kyŏngŭi Line',
    '京元・咸鏡本線': 'Kyŏngwŏn–Hamgyŏng Line',
    '湖南本線': 'Honam Line',
    '全羅線': 'Chŏlla Line',
    '慶全線': 'Kyŏngjŏn Line',
    '東海線': 'East Sea Line',
    ...
    '光州線': 'Kwangju Line',
    '鎭海線': 'Chinhae Line',
    '平壤炭礦線': "P'yŏngyang Colliery Line",
    '滿浦線': "Manp'o Line",
    '平元線': "P'yŏngwŏn Line",
    '龍登線': 'Yongdŭng Line',
    '新義州江岸線': 'Sinŭiju River-bank Line',
    '兼二浦線': "Kyŏmip'o Line",
    '价川線': "Kaech'ŏn Line",
    '平南線': "P'yŏngnam Line",
    '博川線': "Pakch'ŏn Line",
    ...
    '惠山線': 'Hyesan Line',
    '白茂線': 'Paengmu Line',
    '鐵山・遮湖線': "Ch'ŏlsan–Ch'aho Line",
    '川內里線': "Ch'ŏnnaeri Line",
    '水仁線': 'Suin Line',
    '黃海線': 'Hwanghae Line',
    '咸平軌道': "Hamp'yŏng Tramway",
    '水驪線': 'Suryŏ Line',
    '慶北線': 'Kyŏngbuk Line',
    '忠北線': "Ch'ungbuk Line",
    '咸南線': 'Hamnam Line',
    ...
    '長津線': 'Changjin Line',
    '興南線': 'Hŭngnam Line',
    '金剛山電氣鐵道': 'Kŭmgangsan Electric Railway',
    '咸北線': 'Hambuk Line',
    '忠南線': "Ch'ungnam Line",
    '京畿線': 'Kyŏnggi Line',
    '北鮮線': 'Puksŏn Line',
    '會寧炭礦線': 'Hoeryŏng Colliery Line',
    '朝開線': 'Chaokai Line',
    '京圖線': 'Jingtu Line',
    '琿春鐵路線': 'Hunchun Railway',
    '圖佳線': 'Tujia Line',
    '奉山線': 'Fengshan Line',
    '拉濱線': 'Labin Line',
    '錦承線・葉峰線': 'Jincheng–Yefeng Line',
    '京濱線': 'Jingbin Line',
    '奉吉線': 'Fengji Line',
    '平齊線・齊北線・北黑線・濱北線': 'Pingqi–Qibei–Beihei–Binbei Line',
    '濱綏線・濱洲線': 'Binsui–Binzhou Line',
    '連京線': 'Lianjing Line (SMR main line)',
    '營口線': 'Yingkou Line',
    '旅順線': 'Lüshun Line',
    '安奉線': 'Anfeng Line',
    '撫順線': 'Fushun Line',
    '東海道・山陽本線': "Tōkaidō–San'yō Line",
    '關西線・參宮線': 'Kansai–Sangū feeders',
    '關門連絡船': 'Kanmon Ferry',
    '鹿兒島本線': 'Kagoshima Line',
    '長崎本線': 'Nagasaki Line',
    '日豐本線': 'Nippō Line',
    '豐肥本線': 'Hōhi Line',
    '山陰本線': "San'in Line",
    '大社線': 'Taisha Line',
    '宮津線': 'Miyazu Line',
    '北陸・信越・羽越本線': "Hokuriku–Shin'etsu–Uetsu Line",
    '關西本線': 'Kansai Line',
    '東北本線・常磐線・奧羽本線': 'Tōhoku–Jōban–Ōu Line',
    '青函連絡船': 'Seikan Ferry',
    '關釜連絡船': 'Kanpu Ferry',
...
/* A scroll bar for the wide tables, held at the foot of the window.
   Each table scrolls in its own box and that box's bar sits at the bottom of
   it, which on a table taller than the window is off the screen — so a reader
   who needs to see the right-hand columns has to scroll down to find the
   control, use it, and scroll back. This mirrors whichever table is in front
   of them, wherever they are in it. */
#hbar{position:fixed;left:0;right:0;bottom:0;height:15px;overflow-x:auto;
  overflow-y:hidden;z-index:30;background:#00000010;display:none}
#hbar div{height:1px}
body.hbar-on{padding-bottom:20px}
"""
    js = ("(function(){\n"
...
function apply() {
  ...
  document.querySelectorAll('[data-stn]').forEach(function (c) {
    ...
    if (rd) rd.textContent = (rdWant === 'ja' || lang === 'ja') ? r[2]
                           : lang === 'ko' ? r[1] : r[0];
  });
...
/* **The wide tables get a scroll bar at the foot of the window.**
 *
 * Each table scrolls inside its own box, and that box's own bar is at the
 * bottom of the box — which on a table taller than the window is off the
 * screen. So the reader who wants the right-hand columns has to scroll down to
 * find the control, use it, and scroll back up to read the result.
 *
 * This is one bar, fixed at the foot, wired to whichever table is in front of
 * the reader: the one covering the middle of the window, or failing that the
 * first one still on it. It is hidden when that table has nothing to scroll,
 * which is most of them on a wide screen — a control for something that does
 * not move is worse than no control.
 */
(function () {
  ...
  var boxes = [].slice.call(document.querySelectorAll('.tw'));
  var live = null, lock = false;

  function pick() {
    var mid = window.innerHeight / 2, best = null, first = null;
    for (var i = 0; i < boxes.length; i++) {
      var r = boxes[i].getBoundingClientRect();
      if (r.bottom < 0 || r.top > window.innerHeight) continue;
      if (!first) first = boxes[i];
      if (r.top <= mid && r.bottom >= mid) { best = boxes[i]; break; }
    }
    return best || first;
  }
  ...
  function sync() {
    var b = pick();
    var wide = b && b.scrollWidth > b.clientWidth + 2;
    if (!wide) {
      bar.style.display = 'none';
      document.body.classList.remove('hbar-on');
      live = null;
      return;
    }
    live = b;
    ...
    pad.style.width = b.scrollWidth + 'px';
    document.body.classList.add('hbar-on');
    if (!lock) { lock = true; bar.scrollLeft = b.scrollLeft; lock = false; }
  }

  bar.addEventListener('scroll', function () {
    if (lock || !live) return;
    lock = true; live.scrollLeft = bar.scrollLeft; lock = false;
  });
  boxes.forEach(function (b) {
    b.addEventListener('scroll', function () {
      if (lock || live !== b) return;
